       ...
       [
           core_events.Concurrence(
               [
                   ...
                   core_events.Consecution([core_events.SimpleEvent(1)]),
               ]
           ),
...
>>> event[0]
Concurrence([SimpleEvent(duration = DirectDuration(duration = 4/3)), Consecution([SimpleEvent(duration = DirectDuration(duration = 4/3))])])
>>> # We can also get the second leaf of our first leaf
>>> event[0][1]
Consecution([SimpleEvent(duration = DirectDuration(duration = 4/3))])
...
>>> event[1]
Consecution([SimpleEvent(duration = DirectDuration(duration = 4/3)), SimpleEvent(duration = DirectDuration(duration = 4/3))])
>>> # And let's add one more SimpleEvent after those two
>>> event[1].append(core_events.SimpleEvent(1))
>>> event[1]
Consecution([SimpleEvent(duration = DirectDuration(duration = 4/3)), SimpleEvent(duration = DirectDuration(duration = 4/3)), SimpleEvent(duration = DirectDuration(duration = 1))])
